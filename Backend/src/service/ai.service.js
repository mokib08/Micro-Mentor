...
async function generateGoal(mainGoal) {

  // prompt/contents make
  // contents: [
  //     {
  //       role: "user",
  //       parts: [
  //         {
  //           text: `Break down the goal "${mainGoal}" into 5 small actionable micro-goals.

  //             - If the output is naturally a list or steps, return it as a **numbered Markdown list** like:
  //             1. Title – Description – Deadline: "YYYY-MM-DD"
  //             2. Title – Description – Deadline: "YYYY-MM-DD"

  //             - Otherwise return normal plain text (no JSON).
  //             `
  //         }
  //       ]
  //     }
  ...
  const content = `Return **only raw JSON**, no code fences, no text, for this goal:
"${mainGoal}". Format: [{"title":"...","description":"...","deadline":"YYYY-MM-DD"}]`;
  ...
  const microGoals = JSON.parse(text)
  return microGoals
